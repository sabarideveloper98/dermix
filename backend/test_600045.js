...
const run = async () => {
  ...
  if (data.data && data.data.available_courier_companies) {
    ...
    // Check all couriers
    couriers.forEach(c => {
      console.log(c.courier_name, c.rate, c.freight_charge, c.cod_charges, c.smart_order_charge);
    });

    const minRateCourier = couriers.reduce((prev, curr) => prev.rate < curr.rate ? prev : curr);
    console.log("Min Courier:", minRateCourier.courier_name, minRateCourier.rate);
  }
}
run();
